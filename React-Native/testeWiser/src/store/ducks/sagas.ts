...
export  function* auth(params:fetchAction){
    try{
        ...
        const authentication = response.data.filter((item:User)=> {
            return(item.email == params.payload.email && item.password == params.payload.password)
        })
        if(authentication.length > 0){ yield put(Validation({submitted: true, isValid: true}));}
        else{
